...
{{- define "consul.image" -}}
  {{- if .Values.deployDescriptor -}}
    {{- if .Values.consul -}}
      {{- printf "%s" .Values.consul -}}
    {{- else -}}
      {{- printf "%s" (index .Values.deployDescriptor.consul.image) -}}
    {{- end -}}
  {{- else -}}
...
{{- define "consul-k8s.image" -}}
  {{- if .Values.deployDescriptor -}}
    {{- if .Values.consulK8s -}}
      {{- printf "%s" .Values.consulK8s -}}
    {{- else -}}
      {{- printf "%s" (index .Values.deployDescriptor.consulK8s.image) -}}
    {{- end -}}
  {{- else -}}
...
{{- define "consul-dataplane.image" -}}
  {{- if .Values.deployDescriptor -}}
    {{- if .Values.consulDataplane -}}
      {{- printf "%s" .Values.consulDataplane -}}
    {{- else -}}
      {{- printf "%s" (index .Values.deployDescriptor.consulDataplane.image) -}}
    {{- end -}}
  {{- else -}}
...
{{- define "disaster-recovery.image" -}}
  {{- if .Values.deployDescriptor -}}
    {{- if .Values.consulDisasterRecovery -}}
      {{- printf "%s" .Values.consulDisasterRecovery -}}
    {{- else -}}
      {{- printf "%s" (index .Values.deployDescriptor.consulDisasterRecovery.image) -}}
    {{- end -}}
  {{- else -}}
...
{{- define "consul-backup-daemon.image" -}}
  {{- if .Values.deployDescriptor -}}
    {{- if .Values.consulBackupDaemon -}}
      {{- printf "%s" .Values.consulBackupDaemon -}}
    {{- else -}}
      {{- printf "%s" (index .Values.deployDescriptor.consulBackupDaemon.image) -}}
    {{- end -}}
  {{- else -}}
...
{{- define "consul-acl-configurator-operator.image" -}}
  {{- if .Values.deployDescriptor -}}
    {{- if .Values.consulAclConfiguratorOperator -}}
      {{- printf "%s" .Values.consulAclConfiguratorOperator -}}
    {{- else -}}
      {{- printf "%s" (index .Values.deployDescriptor.consulAclConfiguratorOperator.image) -}}
    {{- end -}}
  {{- else -}}
...
{{- define "consul-acl-configurator-rest-server.image" -}}
  {{- if .Values.deployDescriptor -}}
    {{- if .Values.consulAclConfiguratorRestServer -}}
      {{- printf "%s" .Values.consulAclConfiguratorRestServer -}}
    {{- else -}}
      {{- printf "%s" (index .Values.deployDescriptor.consulAclConfiguratorRestServer.image) -}}
    {{- end -}}
  {{- else -}}
...
* SaaS/App deployer (or groovy.deploy.v3) from .Values.deployDescriptor "consul-service" "image"
...
{{- define "consul-integration-tests.image" -}}
  {{- if .Values.deployDescriptor -}}
    {{- if index .Values.deployDescriptor "consul-service" -}}
      {{- printf "%s" (index .Values.deployDescriptor "consul-service" "image") -}}
    {{- else -}}
...
{{- define "consul.monitoredImages" -}}
  {{- if .Values.consulAclConfigurator.enabled -}}
    {{- printf "deployment %s-operator consul-acl-configurator-operator %s, " (include "consul-acl-configurator.name" .) (include "consul.findImage" (list . "consul-acl-configurator-operator")) -}}
    {{- printf "deployment %s-operator consul-acl-configurator-rest-server %s, " (include "consul-acl-configurator.name" .) (include "consul.findImage" (list . "rest-acl-configurator")) -}}
  {{- end -}}
  {{- if .Values.server.enabled -}}
    {{- printf "statefulset %s-server consul %s, " (include "consul.fullname" .) (include "consul.findImage" (list . "consul")) -}}
  {{- end -}}
  {{- if .Values.client.enabled -}}
    {{- printf "daemonset %s consul %s, " (include "consul.fullname" .) (include "consul.findImage" (list . "consul")) -}}
  {{- end -}}
  {{- if eq (include "consul.enableDisasterRecovery" .) "true" -}}
    {{- printf "deployment %s-disaster-recovery consul-disaster-recovery %s, " (include "consul.fullname" .) (include "consul.findImage" (list . "prod.platform.streaming_consul-service")) -}}
  {{- end -}}
  {{- if .Values.backupDaemon.enabled -}}
    {{- printf "deployment %s-backup-daemon consul-backup-daemon %s, " (include "consul.fullname" .) (include "consul.findImage" (list . "prod.platform.streaming_consul-backup-daemon")) -}}
  {{- end -}}
  {{- if .Values.integrationTests.enabled -}}
    {{- printf "deployment %s %s %s, " (include "consul-integration-tests.name" .) (include "consul-integration-tests.name" .) (include "consul.findImage" (list . "consul-service")) -}}
  {{- end -}}
{{- end -}}
