...
are applied to both the Consul server PVCs and the backup daemon PVC. Server PVCs additionally merge in the
per-server `server.persistence.annotations` (server-specific values take precedence on conflict), and the merged set
is added to the server StatefulSet `volumeClaimTemplates`.
...
New PVCs get the annotations at creation time. Existing PVCs are reconciled differently depending on ownership:

* The backup daemon PVC is managed by Helm, so its annotations are updated automatically on `helm upgrade`
  (added and removed) — no additional action is required.
* The consul server PVCs are owned by the StatefulSet (not by Helm), so changing their annotations requires
  `resourceMigration.enabled: true`. On upgrade, a pre-upgrade `resource-migrator` Job reads the desired annotations
  from the server StatefulSet `volumeClaimTemplates` and patches the existing server PVCs to match (adding new keys and
  removing keys that are no longer configured). The StatefulSet is then recreated with the new templates, so PVCs
  created later (for example, on scale-up) also get the correct annotations.
...
| `server.persistence.annotations`           | object  | no        | {}                                                                            | The annotations added to the Consul server PersistentVolumeClaims (and StatefulSet `volumeClaimTemplates`). Merged with `pvc.metadata.annotations`; server-specific values take precedence on conflict.                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| `ui.envoyGateway.enabled`    | boolean | no        | false                                                                 | Enables creation of a Gateway API HTTPRoute resource to expose the Consul UI via Envoy Gateway.                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `ui.envoyGateway.host`       | string  | no        | ""                                                                    | Specifies the hostname used in the generated HTTPRoute for accessing the Consul UI. if empty, `ui.ingress.hosts` will be used.                                                                                                                                                                                                                                                                                                                                                                                           |
| `ui.envoyGateway.parentRefs` | list    | no        | [{"name": "default-external-gateway", "namespace": "gateway-system"}] | List of Gateway resources for the HTTPRoute.                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| `backupDaemon.resources.limits.memory`                          | string  | no        | 512Mi                                                                         | The maximum amount of memory the Consul backup daemon container should use.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `pvc.metadata.annotations`                          | object  | no        | {}                       | The annotations added to all Consul PersistentVolumeClaims (server and backup daemon). Server PVCs annotations (merged with the per-server `server.persistence.annotations`) are added to the StatefulSet `volumeClaimTemplates`; standalone PVCs (backup daemon) receive only these global annotations. Example use case: set `argocd.argoproj.io/sync-options: Prune=false` to prevent ArgoCD from deleting PVCs during sync.                                                                                                                      |
| `resourceMigration.enabled`                         | boolean | no        | false                    | When set to `true`, a pre-upgrade `resource-migrator` Job runs before every `helm upgrade` to reconcile annotations on the existing consul-server PVCs (which are owned by the StatefulSet, not by Helm): new keys are added and stale keys (removed from `pvc.metadata.annotations` or `server.persistence.annotations`) are deleted. The backup daemon PVC is managed by Helm and updated automatically, so it does not require this job. Required to change annotations on already-existing server PVCs. See [PVC Annotations](#pvc-annotations). |
