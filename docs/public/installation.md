...
| `server.persistence.annotations`           | object  | no        | {}                                                                            | The annotations added to the Consul server PersistentVolumeClaims. Merged with `pvc.metadata.annotations`; server-specific values take precedence on conflict.                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `backupDaemon.persistence.annotations`                          | object  | no        | {}                                                                            | The annotations added to the Consul backup daemon PersistentVolumeClaim. Merged with `pvc.metadata.annotations`; backup-daemon-specific values take precedence on conflict.                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `pvc.metadata.annotations`                          | object  | no        | {}                       | The annotations added to all Consul PersistentVolumeClaims. Merged with per-component `persistence.annotations`; component-specific values take precedence on conflict. Example use case: set `argocd.argoproj.io/sync-options: Prune=false` to prevent ArgoCD from deleting PVCs during sync.                                                             |
| `resourceMigration.enabled`                         | boolean | no        | false                    | When set to `true`, a pre-upgrade Job runs before every `helm upgrade` to reconcile annotations on existing PVCs: new keys are added and stale keys (removed from `pvc.metadata.annotations` or `persistence.annotations`) are deleted. Required to remove previously set annotations from already-existing PVCs.                                           |
