...
* The consul server PVCs are owned by the StatefulSet (not by Helm), so changing their annotations requires
  `resourceMigration.enabled: true`. On upgrade, a pre-upgrade `resource-migrator` Job compares the actual annotations
  from the server StatefulSet `volumeClaimTemplates` with the desired `pvc.metadata.annotations`. If they differ, it
  patches the existing server PVCs to match (adding new keys and removing keys that are no longer configured) and
  deletes the server StatefulSet with `--cascade=orphan` (pods and PVCs are kept), because `volumeClaimTemplates` are
  immutable and the upgrade would fail otherwise. Helm then recreates the StatefulSet with the new templates, so PVCs
