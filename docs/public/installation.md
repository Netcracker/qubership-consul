...
[Deployment Status Provisioner](https://git.netcracker.com/PROD.Platform.Streaming/deployment-status-provisioner/-/blob/master/README.md).
...
3. Navigate to the Application Deployer or Groovy Deployer job and specify the following data for build:
   ...
   * `ARTIFACT_DESCRIPTOR_VERSION` is the version of consul-service. It should be provided in the format, 
     `consul-service:x.x.x_delivery_x.x.x_timestamp`. 
      <!-- #GFCFilterMarkerStart# -->The all versions are available on 
      [Release Page](https://git.netcracker.com/PROD.Platform.Streaming/consul-service/-/releases)
      <!-- #GFCFilterMarkerEnd# -->.
   * `DEPLOY_MODE` is the mode of the deployment procedure. It can be `Rolling Update` or `Clean Install`.
      ...
      Never use it for upgrades on production. **Note: Consul does not support upgrade with `Clean Install` mode.**
...
helm delete consul-service-<namespace_name> --namespace=<namespace_name>
```

Then update `release` annotations for secrets and persistent volume claims:

```bash
kubectl get secret -n <namespace_name> -o json | jq '.items[].metadata|select(.annotations."meta.helm.sh/release-name")|.name' | awk '{print "kubectl annotate --overwrite secret", $1, "meta.helm.sh/release-name=consul-service -n <namespace_name>"}' | bash -x
kubectl get pvc -n <namespace_name> -o json | jq '.items[].metadata|select(.annotations."meta.helm.sh/release-name")|.name' | awk '{print "kubectl annotate --overwrite pvc", $1, "meta.helm.sh/release-name=consul-service -n <namespace_name>"}' | bash -x
```

And finally install Consul service using App Deployer.
...
```bash
kubectl delete all,configmaps,ingresses,serviceaccounts,roles,rolebindings,grafanadashboard,prometheusrule,servicemonitor,certificates,sitemanager --all --namespace=<namespace_name>
kubectl get clusterrole,clusterrolebinding,podsecuritypolicy | awk '{print $1}' | grep <global.name> | xargs kubectl delete
kubectl delete secrets -l release=consul-service --namespace=<namespace_name>
```

If you use storage classes for Consul persistent volume claims, you need to actualize annotations and labels only for
`pvc-<name>-snapshots` persistent volume claim:

```bash
kubectl annotate --overwrite pvc data-<global.name>-backup-daemon meta.helm.sh/release-name=consul-service meta.helm.sh/release-namespace=<namespace_name> -n <namespace_name>
kubectl label --overwrite pvc data-<global.name>-backup-daemon app.kubernetes.io/managed-by=Helm -n <namespace_name>
```

If you use predefined persistent volumes for Consul, you need to actualize annotations and labels for all persistent volume claims:

```bash
kubectl annotate --overwrite pvc --all meta.helm.sh/release-name=consul-service meta.helm.sh/release-namespace=<namespace_name> -n <namespace_name>
kubectl label --overwrite pvc --all app.kubernetes.io/managed-by=Helm -n <namespace_name>
```

Then install Consul service using App Deployer and `DEPLOY_W_HELM: true`.

## How to recover Consul after upgrade with App Deployer `Clean Install` mode?

The `Clean Install` mode of App Deployer removes everything from the namespace before the deployment, including Persistent Volumes Claims.
If you have lost Consul persistent volumes, you can't recover its data.

If you have persistent volumes but the secret with Consul bootstrap ACL token is lost, you can re-generate it with the 
[Refresh Bootstrap ACL Token](/docs/public/security.md) guide.
...
(for example, `sh.helm.release.v1.consul-service-consul-service.v1`) from the namespace.
...
1. If you use App Deployer with `DEPLOY_W_HELM: false`, the reason may be in the absence of Deployment Status Provisioner.
   When we set `DEPLOY_W_HELM` to `false`, some important features are not working as expected.
   In that case Status Provisioner cleanup job is created simultaneously with the creation of Status Provisioner job and 
   removes it before it manages to compute cluster statuses. 
   To tackle the problem, you need to add the following section to your parameters list:
...
2. If you use DP Deployer or App Deployer with `DEPLOY_W_HELM: true`, it can be an issue with timeouts or long start of Consul pods.
   You need to get statuses from the `consul-status-provisioner` job resource and analyze them. You can do it in two ways:

   * Look through `consul-status-provisioner` pod logs:

     ```text
     Status Provisioner have started calculating the state of the cluster
     Processing [Deployment consul-backup-daemon] resource
     Processing [StatefulSet consul-server] resource
     Processing [Deployment consul-acl-configurator-operator] resource
     Processing [Deployment consul-integration-tests-runner] resource
     Processing integration tests status from [consul-integration-tests-runner] resource
     Failed components statuses are []
     ```

   * Get content of the `consul-status-provisioner` job resource:

     ```bash
     kubect get job consul-status-provisioner -o yaml
     ```

**Important**: By default, `consul-status-provisioner` job resource remains alive `600` seconds after its completion. 
You can increase this timeout by specifying `statusProvisioner.lifetimeAfterCompletion` parameter value.

You can also increase the pod readiness timeout `statusProvisioner.podReadinessTimeout: 600` and try to run the Deployer job again.

## Deployer job failed with some error in templates

Make sure you performed the necessary [Prerequisites](#prerequisites). Fill the [Parameters](#parameters) correctly and compare with 
[Examples](#on-prem-examples).
...
```text
Error: UPGRADE FAILED: cannot patch "consul-server" with kind StatefulSet: StatefulSet.apps "consul-server" is invalid: spec: Forbidden: updates to statefulset spec for fields other than 'replicas', 'template', 'updateStrategy', 'persistentVolumeClaimRetentionPolicy' and 'minReadySeconds' are forbidden
```

Most often it is associated with persistent volumes configuration. To determine the problem area, in OpenShift/Kubernetes find 
`StatefulSet` configuration using the following command:

```sh
kubectl describe statefulset <global.name>-server -n <namespace_name>
```

Pay attention to the `Volume Claims` section and compare its values to the persistence parameters (`server.storage`, `server.storageClass`)
specified in Deployer job.

```text
Volume Claims:
  Name:          data-consul-service
...
2. Remove the `StatefulSet` resource without deleting the pods:
   ...
   ```sh
   kubectl delete sts <global.name>-server -n <namespace_name> --cascade=orphan
   ```
   
   And run Deployer job with required parameters and `upgrade` mode.

## Deployer job fails with "Error: cannot re-use a name that is still in use" error

This error in the Deployer job means that some incorrect actions cause the situation when Helm release for 
Consul service installation does not exist:
...
but `<namespace_name>` namespace contains secrets connected to the corresponding Helm release:

```sh
$ oc get secret -n <namespace_name> | grep consul | grep release
sh.helm.release.v1.consul-service.v1                       helm.sh/release.v1                    1         8d
sh.helm.release.v1.consul-service.v2                       helm.sh/release.v1                    1         6h
sh.helm.release.v1.consul-service.v3                       helm.sh/release.v1                    1         6h
sh.helm.release.v1.consul-service.v4                       helm.sh/release.v1                    1         6h
sh.helm.release.v1.consul-service.v5                       helm.sh/release.v1                    1         4h
sh.helm.release.v1.consul-service.v6                       helm.sh/release.v1                    1         3h
sh.helm.release.v1.consul-service.v7                       helm.sh/release.v1                    1         3h
```

To solve the problem, remove all secrets connected to the corresponding Helm release.

## Deployer job fails with "Error: cannot patch 'consul-server' with kind Service" error

The following error in the Deployer job means that Kubernetes cannot apply changed list of service ports:

```text
Error: cannot patch "consul-server" with kind Service: Service "consul-server" is invalid: spec.ports[10].name: Duplicate value: "grpc"
```

During upgrade to `1.14.4` Consul ([consul-service 0.2.0](https://git.netcracker.com/PROD.Platform.Streaming/consul-service/-/tags/0.2.0)),
the `grpc` port is changed from `8503` to `8502`.

To solve the problem, update list of ports in Consul server `StatefulSet` and `Service` resources manually or remove 
them using the following commands:

```sh
kubectl delete service <global.name>-server -n <namespace_name>
```
...
## Deployer job fails with 'no matches for kind "PodDisruptionBudget" in version "policy/v1beta1"' error

The following error in the Deployer job means that Kubernetes cannot apply `PodDisruptionBudget` resource with old version:
...
* The version of the Consul service manifest being installed is not certified for the Kubernetes version being used.

  **To solve the problem**, use certified manifest. You can check manifests certified for your Kubernetes in the 
  [Releases and Sprints](https://bass.netcracker.com/display/PLAT/Releases+and+Sprints) article.

* The installation is performed with `DEPLOY_W_HELM` parameter set to `false`.

  **To solve the problem**, reinstall Consul service with `DEPLOY_W_HELM` parameter set to `true` (check 
  [How to deploy with DEPLOY_W_HELM true over false](#how-to-deploy-with-deploy_w_helm-true-over-false) guide), 
  or disable `PodDisruptionBudget` resource creation with `server.disruptionBudget.enabled` parameter set to `false`.
